 ...
 if(message.channel.type === "dm"){
   if(message.author.bot) return;
    message.channel.send(new Discord.RichEmbed()
            .setColor("RANDOM")
            .setTitle(`DİKKAT`)
            .setDescription(`Mesaj geçmişim sahibim tarafından kontrol edilmektedir.`)
             )
 }/////FİBER BOTLİST & 
});
client.on("message", async message => { 
  var anahtar = db.fetch(`everhere_${message.guild.id}`)
  if(anahtar === "acik"){
    if(message.author.bot) return;
    ...
    if(message.content.includes("@everyone") || message.content.includes("@here"))
      {
        message.delete();
        return message.channel.send("Everyone veya here atamazsın.").then(i => i.delete(10000))
      }
...
client.on("message", async msg => {
  let küfür = await db.fetch(`küfür_${msg.guild.id}`)
    if (küfür == "açık") {
        const küfür2 = ["oç", "amk", "ananı sikiyim", "ananıskm", "piç", "amk", "amsk", "sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "mal", "sik", "yarrak", "am", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq", "aq", "ak", "amq", "aw" ,"annesiz",];
